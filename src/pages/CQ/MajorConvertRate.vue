        ...
        >
          ...
          <el-table-column label-class-name="warning-row" label="调剂率" :sortable="true">
            <template slot-scope="{row}">
              ...
              <span v-else>{{ row.major_convert_rate }}</span>
            </template>
          </el-table-column>
...
<script>
import { fetchList,updateMcr,deleteMcr } from "@/api/majorConvertRate";
...
export default {
  ...
  methods: {
    ...
    getData(){
      ...
      fetchList(this.page).then((data)=>{
        ...
        this.tableData = data.result.result.map(v => {
          ...
          v.originalMcr = v.major_convert_rate 
    ...
    deleteEr(){
      deleteMcr(this.deleteItem).then(re=>{
        this.$message({
          type:'success',
          message:re.msg
        })
    ...
    cancelEdit(row) {
      row.major_convert_rate = row.originalMcr
