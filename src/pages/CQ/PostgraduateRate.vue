  ...
  <div>
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="deletePr">确 定</el-button>
      </span>
    </el-dialog>    
    <!--操作栏-->
    <el-row>
    ...
    <el-row>
      <el-col>
        ...
        >
          ...
          <el-table-column align="center" label-class-name="warning-row" label="考研率" :sortable="true">
            <template slot-scope="{row}">
              <template v-if="row.edit">
                <el-input v-model="row.postgraduate_rate" size="mini" class="edit-input"></el-input>
              </template>
              <span v-else>{{ row.postgraduate_rate }}</span>
            </template>
          </el-table-column>
...
<script>
import { fetchList,updatePr,deletePr } from "@/api/postgraduateRate";
...
export default {
  ...
  methods: {
    ...
    getData(){
      ...
      fetchList(this.page).then((data)=>{
        ...
        this.tableData = data.result.result.map(v => {
          ...
          v.originalPr = v.postgraduate_rate 
    ...
    deletePr(){
      deletePr(this.deleteItem).then(re=>{
        this.$message({
          type:'success',
          message:re.msg
        })
    ...
    cancelEdit(row) {
      row.postgraduate_rate = row.originalPr
    ...
    confirmEdit(row) {
      row.edit = false;
      row.originalPr = row.postgraduate_rate 
      row.originalRemark = row.remarks      
      let tmp = {...row}
      delete tmp.edit
      delete tmp.originalPr
      delete tmp.originalRemark
      updatePr(tmp).then(re=>{
        this.$message({
          message: re.msg,
          type: "success"
        });
