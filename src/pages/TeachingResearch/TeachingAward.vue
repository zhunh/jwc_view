  ...
  <div>
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="deleteTa">确 定</el-button>
      </span>
    </el-dialog>    
    <!--操作栏-->
    <el-row>
    ...
    <el-row>
      <el-col>
        ...
        >
          ...
          <el-table-column label-class-name="warning-row" label="教学成果奖" :sortable="true">
            <template slot-scope="{row}">
              <template v-if="row.edit">
                <el-input v-model="row.teaching_achievement_award" size="mini" class="edit-input"></el-input>
              </template>
              <span v-else>{{ row.teaching_achievement_award }}</span>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getData(){
      ...
      fetchList(this.page).then((data)=>{
        ...
        this.tableData = data.result.result.map(v => {
          ...
          v.originalTa = v.teaching_achievement_award 
    ...
    deleteTa(){
      deleteTa(this.deleteItem).then(re=>{
        this.$message({
          type:'success',
          message:re.msg
        })
