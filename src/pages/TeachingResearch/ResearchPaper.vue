  ...
  <div>
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="deleteRp">确 定</el-button>
      </span>
    </el-dialog>    
    <!--操作栏-->
    <el-row>
    ...
    <el-row>
      <el-col>
        ...
        >
          ...
          <el-table-column align="center" label-class-name="warning-row" label="教改论文" :sortable="true">
            <template slot-scope="{row}">
              <template v-if="row.edit">
                <el-input v-model="row.research_paper" size="mini" class="edit-input"></el-input>
              </template>
              <span v-else>{{ row.research_paper }}</span>
            </template>
          </el-table-column>
...
<script>
import { fetchList,updateRp,deleteRp } from "@/api/researchPaper";
...
export default {
  ...
  methods: {
    ...
    deleteRp(){
      deleteRp(this.deleteItem).then(re=>{
        this.$message({
          type:'success',
          message:re.msg
        })
    ...
    cancelEdit(row) {
      row.research_paper = row.originalRp
    ...
    confirmEdit(row) {
      row.edit = false;
      row.originalRp = row.research_paper 
      row.originalRemark = row.remarks      
      let tmp = {...row}
      delete tmp.edit
      delete tmp.originalRp
      delete tmp.originalRemark
      updateRp(tmp).then(re=>{
        this.$message({
          message: re.msg,
          type: "success"
        });
