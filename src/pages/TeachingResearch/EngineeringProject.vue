  ...
  <div>
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="deleteEp">确 定</el-button>
      </span>
    </el-dialog>    
    <!--操作栏-->
    <el-row>
    ...
    <el-row>
      <el-col>
        ...
        >
          ...
          <el-table-column label-class-name="warning-row" label="教学主持本科工程" :sortable="true">
            <template slot-scope="{row}">
              <template v-if="row.edit">
                <el-input v-model="row.engineering_project_num" size="mini" class="edit-input"></el-input>
              </template>
              <span v-else>{{ row.engineering_project_num }}</span>
            </template>
          </el-table-column>
...
<script>
import { fetchList,updateEp,deleteEp } from "@/api/engineeringProject";
...
export default {
  ...
  methods: {
    ...
    getData(){
      ...
      fetchList(this.page).then((data)=>{
        ...
        this.tableData = data.result.result.map(v => {
          ...
          v.originalEp = v.engineering_project_num 
    ...
    deleteEp(){
      deleteEp(this.deleteItem).then(re=>{
        this.$message({
          type:'success',
          message:re.msg
        })
    ...
    cancelEdit(row) {
      row.engineering_project_num = row.originalEp
    ...
    confirmEdit(row) {
      row.edit = false;
      row.originalEp = row.engineering_project_num 
      row.originalRemark = row.remarks      
      let tmp = {...row}
      delete tmp.edit
      delete tmp.originalEp
      delete tmp.originalRemark
      updateEp(tmp).then(re=>{
        this.$message({
          message: re.msg,
          type: "success"
        });
